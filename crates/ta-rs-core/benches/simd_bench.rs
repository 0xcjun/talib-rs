...
fn bench_sum_sq_diff(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000] {
        ...
        group.bench_with_input(BenchmarkId::new("scalar", size), &data, |b, d| {
            b.iter(|| {
                let m = mean;
                let sum: f64 = black_box(d).iter().map(|&x| { let diff = x - m; diff * diff }).sum();
                sum
            });
        });
    }
...
fn bench_add_arrays(c: &mut Criterion) {
    ...
    for size in [1000, 10000, 100000] {
        ...
        group.bench_with_input(BenchmarkId::new("scalar", size), &size, |bench, _| {
            bench.iter(|| {
                let r: Vec<f64> = black_box(&a).iter().zip(black_box(&b).iter()).map(|(x, y)| x + y).collect();
                r
            });
        });
    }
...
fn bench_indicators(c: &mut Criterion) {
    let close: Vec<f64> = (0..10000).map(|i| 100.0 + (i as f64 * 0.01).sin() * 10.0).collect();

    let mut group = c.benchmark_group("indicators_10k");
    group.bench_function("SMA_20", |b| {
        b.iter(|| ta_rs_core::overlap::sma(black_box(&close), 20));
    });
    group.bench_function("EMA_20", |b| {
        b.iter(|| ta_rs_core::overlap::ema(black_box(&close), 20));
    });
    group.bench_function("RSI_14", |b| {
        b.iter(|| ta_rs_core::momentum::rsi(black_box(&close), 14));
    });
    group.bench_function("BBANDS_20", |b| {
        b.iter(|| ta_rs_core::overlap::bbands(black_box(&close), 20, 2.0, 2.0, ta_rs_core::MaType::Sma));
    });
    group.bench_function("STDDEV_20", |b| {
        b.iter(|| ta_rs_core::statistic::stddev(black_box(&close), 20, 1.0));
    });
    group.finish();
}

criterion_group!(benches, bench_sum, bench_sum_sq_diff, bench_add_arrays, bench_sqrt_array, bench_indicators);
