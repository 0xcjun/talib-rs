...
pub fn cdl_2crows(
    ...
    for i in 2..len {
        // 第一根阳线(上升趋势)
        if !is_bullish(open[i - 2], close[i - 2]) { continue; }
        let avg = body_avg(open, close, i - 2);
        if real_body(open[i - 2], close[i - 2]) < avg { continue; }
        // 第二根阴线跳空高开
        if is_bullish(open[i - 1], close[i - 1]) { continue; }
        if open[i - 1] <= close[i - 2] { continue; }
        // 第三根阴线: 开盘在第二根实体内, 收盘在第一根实体内
        if is_bullish(open[i], close[i]) { continue; }
        if open[i] >= open[i - 1] || open[i] <= close[i - 1] { continue; }
        if close[i] >= open[i - 2] || close[i] <= close[i - 2] { continue; }
        output[i] = -100;
    }
    Ok(output)
}
...
pub fn cdl_counterattack(
    ...
    for i in 1..len {
        ...
        // 方向相反 + 收盘价接近 + 实体都较大
        if prev_bull == curr_bull { continue; }
        if !near_eq(close[i - 1], close[i], tol) { continue; }
        if real_body(open[i - 1], close[i - 1]) < avg * 0.5 { continue; }
        if real_body(open[i], close[i]) < avg * 0.5 { continue; }
        // 多头反击: 前阴后阳
...
pub fn cdl_dojistar(
    ...
    for i in 1..len {
        ...
        let prev_body = real_body(open[i - 1], close[i - 1]);
        if prev_body < avg { continue; }
        if !is_doji(open[i], high[i], low[i], close[i], 0.1) { continue; }
        // 跳空判断
...
pub fn cdl_harami(
    ...
    for i in 1..len {
        ...
        let curr_body = real_body(open[i], close[i]);
        let avg = body_avg(open, close, i - 1);
        if prev_body < avg * 0.5 || curr_body >= prev_body { continue; }
...
pub fn cdl_hikkakemod(
    ...
    for i in 2..len.saturating_sub(3) {
        ...
        // i-1 和 i 构成内包线
        if high[i] >= high[i - 1] || low[i] <= low[i - 1] { continue; }
        if high[i - 1] >= high[i - 2] || low[i - 1] <= low[i - 2] { continue; }
        for j in (i + 1)..=(i + 3).min(len - 1) {
            if close[j] > high[i] {
                output[j] = 100;
                break;
...
pub fn cdl_homingpigeon(
    ...
    for i in 1..len {
        // 两根都是阴线
        if is_bullish(open[i - 1], close[i - 1]) || is_bullish(open[i], close[i]) { continue; }
        let avg = body_avg(open, close, i - 1);
        if real_body(open[i - 1], close[i - 1]) < avg { continue; }
        // 第二根实体在第一根内
...
pub fn cdl_kicking(
    ...
    for i in 1..len {
        ...
        if range1 <= 0.0 || range2 <= 0.0 { continue; }
...
pub fn cdl_separatinglines(
    ...
    for i in 1..len {
        ...
        let bull2 = is_bullish(open[i], close[i]);
        if bull1 == bull2 { continue; }
        if !near_eq(open[i - 1], open[i], tol) { continue; }
        if real_body(open[i], close[i]) < avg * 0.5 { continue; }
        output[i] = if bull2 { 100 } else { -100 };
    }
    Ok(output)
...
pub fn cdl_3blackcrows(
    ...
    for i in 2..len {
        ...
        if is_bullish(open[i], close[i]) { continue; }
        // 每根收盘都更低
        if close[i - 1] >= close[i - 2] || close[i] >= close[i - 1] { continue; }
        // 每根开盘在前一根实体内
        if open[i - 1] > open[i - 2] || open[i - 1] < close[i - 2] { continue; }
        if open[i] > open[i - 1] || open[i] < close[i - 1] { continue; }
        let avg = body_avg(open, close, i);
        // 实体都较大
...
pub fn cdl_3inside(
    ...
    for i in 2..len {
        ...
        // 孕线: 第二根在第一根内
        if top2 > top1 || bot2 < bot1 { continue; }
        let avg = body_avg(open, close, i - 2);
        if real_body(open[i - 2], close[i - 2]) < avg * 0.5 { continue; }
        // 三内部上升: 前阴+后阳孕线 + 第三根阳线收盘高于第一根开盘
        if !is_bullish(open[i - 2], close[i - 2]) && is_bullish(open[i - 1], close[i - 1])
            && is_bullish(open[i], close[i]) && close[i] > top1
        {
            output[i] = 100;
        }
        // 三内部下降: 前阳+后阴孕线 + 第三根阴线收盘低于第一根开盘
        else if is_bullish(open[i - 2], close[i - 2]) && !is_bullish(open[i - 1], close[i - 1])
            && !is_bullish(open[i], close[i]) && close[i] < bot1
        {
            output[i] = -100;
        }
    }
...
pub fn cdl_3outside(
    ...
    for i in 2..len {
        ...
        if !is_bullish(open[i - 2], close[i - 2]) && is_bullish(open[i - 1], close[i - 1])
            ...
            && is_bullish(open[i], close[i]) && close[i] > close[i - 1]
        {
            output[i] = 100;
        }
        // 空头吞没 + 阴线确认
...
pub fn cdl_3starsinsouth(
    ...
    for i in 2..len {
        ...
        if b2 / range2 < 0.5 { continue; }
        // 低点递增
        if low[i - 1] < low[i - 2] || low[i] < low[i - 1] { continue; }
        output[i] = 100; // 看涨反转
    }
    Ok(output)
}
...
pub fn cdl_abandonedbaby(
    ...
    for i in 2..len {
        // 中间是十字星
        if !is_doji(open[i - 1], high[i - 1], low[i - 1], close[i - 1], 0.1) { continue; }
        let avg = body_avg(open, close, i);
        // 第一根和第三根实体较大
        if real_body(open[i - 2], close[i - 2]) < avg * 0.5 { continue; }
        if real_body(open[i], close[i]) < avg * 0.5 { continue; }
...
pub fn cdl_breakaway(
    ...
    for i in 4..len {
        ...
        if !is_bullish(open[i - 4], close[i - 4]) && real_body(open[i - 4], close[i - 4]) > avg {
            if high[i - 3] < close[i - 4] { // 向下跳空
                if is_bullish(open[i], close[i]) && close[i] > close[i - 4] && close[i] < open[i - 4] {
                    output[i] = 100;
                }
            }
        }
        // 空头脱离: 长阳线 + 跳空向上小K线 + 两根小K线 + 阴线收在第一根实体内
        else if is_bullish(open[i - 4], close[i - 4]) && real_body(open[i - 4], close[i - 4]) > avg {
            if low[i - 3] > close[i - 4] { // 向上跳空
                if !is_bullish(open[i], close[i]) && close[i] < close[i - 4] && close[i] > open[i - 4] {
                    output[i] = -100;
                }
            }
...
pub fn cdl_concealbabyswall(
    ...
    for i in 3..len {
        // 四根都是阴线
        if is_bullish(open[i - 3], close[i - 3]) { continue; }
        if is_bullish(open[i - 2], close[i - 2]) { continue; }
        if is_bullish(open[i - 1], close[i - 1]) { continue; }
        if is_bullish(open[i], close[i]) { continue; }
        ...
        if us1 > range1 * 0.05 || ls1 > range1 * 0.05 { continue; }
        if us2 > range2 * 0.05 || ls2 > range2 * 0.05 { continue; }
        // 第三根跳空低开但有长上影线触及第二根实体
        if open[i - 1] >= close[i - 2] { continue; }
        if high[i - 1] < close[i - 2] { continue; }
        // 第四根吞没第三根(包括影线)
        if open[i] < high[i - 1] || close[i] > low[i - 1] { continue; }
        output[i] = 100; // 看涨反转
    }
    Ok(output)
}
...
pub fn cdl_eveningdojistar(
    ...
    for i in 2..len {
        ...
        // 第二根十字星，向上跳空
        if !is_doji(open[i - 1], high[i - 1], low[i - 1], close[i - 1], 0.1) { continue; }
        if low[i - 1] <= close[i - 2] { continue; }
        // 第三根阴线，收盘深入第一根实体
        if is_bullish(open[i], close[i]) { continue; }
        let mid1 = (open[i - 2] + close[i - 2]) / 2.0;
        if close[i] > mid1 { continue; }
        output[i] = -100;
    }
    Ok(output)
}
...
pub fn cdl_eveningstar(
    ...
    for i in 2..len {
        ...
        if real_body(open[i - 2], close[i - 2]) < avg { continue; }
        // 第二根小实体，向上跳空
        if real_body(open[i - 1], close[i - 1]) > avg * 0.3 { continue; }
        let star_bot = open[i - 1].min(close[i - 1]);
        if star_bot <= close[i - 2] { continue; }
        // 第三根阴线
        if is_bullish(open[i], close[i]) { continue; }
        if real_body(open[i], close[i]) < avg * 0.5 { continue; }
        let mid1 = (open[i - 2] + close[i - 2]) / 2.0;
...
pub fn cdl_gapsidesidewhite(
    ...
    for i in 2..len {
        ...
        let tol = avg * 0.3;
        // 两根阳线大小相似
        if (b1 - b2).abs() > tol { continue; }
        // 开盘价接近
        if (open[i - 1] - open[i]).abs() > tol { continue; }
        // 向上跳空
        if low[i - 1] > high[i - 2] {
            output[i] = 100;
        }
...
pub fn cdl_identical3crows(
    ...
    for i in 2..len {
        ...
        let tol = avg * 0.05;
        // 每根开盘等于前一根收盘
        if !near_eq(open[i - 1], close[i - 2], tol) { continue; }
        if !near_eq(open[i], close[i - 1], tol) { continue; }
...
pub fn cdl_ladderbottom(
    ...
    for i in 4..len {
        // 前三根为阴线，收盘递降
        if is_bullish(open[i - 4], close[i - 4]) { continue; }
        if is_bullish(open[i - 3], close[i - 3]) { continue; }
        if is_bullish(open[i - 2], close[i - 2]) { continue; }
        if close[i - 3] >= close[i - 4] || close[i - 2] >= close[i - 3] { continue; }
        // 第四根阴线有长上影线 (上影线 > 实体)
        if is_bullish(open[i - 1], close[i - 1]) { continue; }
        let us = upper_shadow(open[i - 1], high[i - 1], close[i - 1]);
        if us < real_body(open[i - 1], close[i - 1]) { continue; }
        // 第五根阳线，跳空高开
        if !is_bullish(open[i], close[i]) { continue; }
...
pub fn cdl_mathold(
    ...
    for i in 4..len {
        ...
        if is_bullish(open[i - 4], close[i - 4]) && real_body(open[i - 4], close[i - 4]) > avg {
            ...
            if low[i - 3] < hold_low || low[i - 2] < hold_low || low[i - 1] < hold_low { continue; }
            let small = avg * 0.5;
            if real_body(open[i - 3], close[i - 3]) > small { continue; }
            if real_body(open[i - 2], close[i - 2]) > small { continue; }
            if real_body(open[i - 1], close[i - 1]) > small { continue; }
            // 第五根阳线创新高
        ...
        // 空头类似但方向相反
            ...
            if high[i - 3] > hold_high || high[i - 2] > hold_high || high[i - 1] > hold_high { continue; }
            let small = avg * 0.5;
            if real_body(open[i - 3], close[i - 3]) > small { continue; }
            if real_body(open[i - 2], close[i - 2]) > small { continue; }
            if real_body(open[i - 1], close[i - 1]) > small { continue; }
...
pub fn cdl_morningstar(
    ...
    for i in 2..len {
        ...
        if star_top >= close[i - 2] { continue; }
        // 第三根阳线
        if !is_bullish(open[i], close[i]) { continue; }
        if real_body(open[i], close[i]) < avg * 0.5 { continue; }
        let mid1 = (open[i - 2] + close[i - 2]) / 2.0;
    ...
    for i in 4..len {
        ...
        if is_bullish(open[i - 4], close[i - 4]) && real_body(open[i - 4], close[i - 4]) > avg {
            ...
            let mid_ok = (1..=3).all(|k| {
                let j = i - 4 + k;
                real_body(open[j], close[j]) < avg * 0.5 && low[j] >= first_low && high[j] <= first_high
            });
...
pub fn cdl_tasukigap(
    ...
    for i in 2..len {
        ...
        if is_bullish(open[i - 2], close[i - 2]) && is_bullish(open[i - 1], close[i - 1]) {
            if low[i - 1] > high[i - 2] { // 向上跳空
                if !is_bullish(open[i], close[i])
                    && open[i] >= open[i - 1] && open[i] <= close[i - 1]
                    && close[i] < open[i - 1] && close[i] > high[i - 2]
                {
                    output[i] = 100; // 看涨延续
                }
            }
        ...
        if !is_bullish(open[i - 2], close[i - 2]) && !is_bullish(open[i - 1], close[i - 1]) {
            if high[i - 1] < low[i - 2] { // 向下跳空
                if is_bullish(open[i], close[i])
                    && open[i] <= open[i - 1] && open[i] >= close[i - 1]
                    && close[i] > open[i - 1] && close[i] < low[i - 2]
                {
                    output[i] = -100; // 看跌延续
                }
            }
...
pub fn cdl_tristar(
    ...
    let mut output = vec![0i32; len];
    for i in 2..len {
        if !is_doji(open[i - 2], high[i - 2], low[i - 2], close[i - 2], 0.1) { continue; }
        if !is_doji(open[i - 1], high[i - 1], low[i - 1], close[i - 1], 0.1) { continue; }
        if !is_doji(open[i], high[i], low[i], close[i], 0.1) { continue; }
        let mid1 = (open[i - 2] + close[i - 2]) / 2.0;
...
pub fn cdl_unique3river(
    ...
    for i in 2..len {
        ...
        let top2 = open[i - 1];
        let bot2 = close[i - 1];
        if top2 > open[i - 2] || bot2 < close[i - 2] { continue; }
        if low[i - 1] >= low[i - 2] { continue; } // 需要创新低
        let ls = lower_shadow(open[i - 1], low[i - 1], close[i - 1]);
        if ls < real_body(open[i - 1], close[i - 1]) { continue; }
        // 第三根小阳线，收盘不高于第二根收盘
        if !is_bullish(open[i], close[i]) { continue; }
        if close[i] > bot2 { continue; }
        output[i] = 100;
    }
    Ok(output)
}
...
pub fn cdl_upsidegap2crows(
    ...
    for i in 2..len {
        ...
        // 第二根小阴线，向上跳空
        if is_bullish(open[i - 1], close[i - 1]) { continue; }
        if close[i - 1] <= close[i - 2] { continue; } // 跳空
        // 第三根阴线吞没第二根，收盘仍高于第一根收盘
        if is_bullish(open[i], close[i]) { continue; }
        if open[i] <= open[i - 1] || close[i] >= close[i - 1] { continue; }
