...
impl HtState {
    fn new(len: usize, need_phasor: bool) -> Self {
        Self {
            ...
            i1_out: if need_phasor {
                vec![0.0; len]
            } else {
                Vec::new()
            },
            q1_out: if need_phasor {
                vec![0.0; len]
            } else {
                Vec::new()
            },
...
fn ht_core(input: &[f64], compute_dc_phase: bool, need_phasor: bool) -> HtState {
    ...
    for i in 0..len {
        ...
        if i >= 9 {
            ...
            // Detrender
            let det_val = (a * s.smooth.get(i) + b * s.smooth.get(i.wrapping_sub(2))
            ...
            let q1_val = (a * s.detrender.get(i) + b * s.detrender.get(i.wrapping_sub(2))
            ...
            // 推进 90 度相位：jI 和 jQ
            let ji_val = (a * s.i1.get(i) + b * s.i1.get(i.wrapping_sub(2))
                - b * s.i1.get(i.wrapping_sub(4))
                - a * s.i1.get(i.wrapping_sub(6)))
                * adj;
            s.ji.set(i, ji_val);
            let jq_val = (a * s.q1.get(i) + b * s.q1.get(i.wrapping_sub(2))
            ...
            let mut re_val = s.i2.get(i) * s.i2.get(i.wrapping_sub(1))
                + s.q2.get(i) * s.q2.get(i.wrapping_sub(1));
            let mut im_val = s.i2.get(i) * s.q2.get(i.wrapping_sub(1))
                - s.q2.get(i) * s.i2.get(i.wrapping_sub(1));
            ...
            s.smooth_period[i] = 0.33 * period_val + 0.67 * s.smooth_period[i.saturating_sub(1)];
