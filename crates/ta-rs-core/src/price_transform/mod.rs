...
pub fn avgprice(
    ...
    Ok((0..len).map(|i| (open[i] + high[i] + low[i] + close[i]) / 4.0).collect())
}
...
pub fn typprice(high: &[f64], low: &[f64], close: &[f64]) -> TaResult<Vec<f64>> {
    ...
    Ok((0..len).map(|i| (high[i] + low[i] + close[i]) / 3.0).collect())
}
...
pub fn wclprice(high: &[f64], low: &[f64], close: &[f64]) -> TaResult<Vec<f64>> {
    ...
    Ok((0..len).map(|i| (high[i] + low[i] + 2.0 * close[i]) / 4.0).collect())
}
