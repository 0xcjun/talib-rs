    ...
    if adx_start < len {
        ...
        if count > 0 {
            ...
            for i in (adx_start + 1)..len {
                if !dx_values[i].is_nan() {
                    prev_adx = (prev_adx * (timeperiod as f64 - 1.0) + dx_values[i]) / timeperiod as f64;
                    output[i] = prev_adx;
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_adx_basic() {
        let high: Vec<f64> = (0..50).map(|i| 50.0 + (i as f64 * 0.3).sin() * 5.0 + 2.0).collect();
        let low: Vec<f64> = (0..50).map(|i| 50.0 + (i as f64 * 0.3).sin() * 5.0 - 2.0).collect();
        let close: Vec<f64> = (0..50).map(|i| 50.0 + (i as f64 * 0.3).sin() * 5.0).collect();
