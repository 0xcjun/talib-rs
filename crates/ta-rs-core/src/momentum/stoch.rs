...
pub fn stochrsi(
    ...
    for i in (fastk_period - 1)..rsi_len {
        ...
        let mut ll = f64::INFINITY;
        for j in start..=i {
            if rsi_valid[j] > hh { hh = rsi_valid[j]; }
            if rsi_valid[j] < ll { ll = rsi_valid[j]; }
        }
        let range = hh - ll;
        if range > 0.0 {
...
mod tests {
    ...
    #[test]
    fn test_stoch_basic() {
        let high: Vec<f64> = (0..30).map(|i| 50.0 + (i as f64 * 0.3).sin() * 5.0 + 2.0).collect();
        let low: Vec<f64> = (0..30).map(|i| 50.0 + (i as f64 * 0.3).sin() * 5.0 - 2.0).collect();
