    ...
    for i in 0..len {
        ...
        if i >= 9 {
            ...
            detrender[i] = (a * smooth[i] + b * smooth[i.saturating_sub(2)]
                - b * smooth[i.saturating_sub(4)] - a * smooth[i.saturating_sub(6)]) * adj;

            q1[i] = (a * detrender[i] + b * detrender[i.saturating_sub(2)]
                - b * detrender[i.saturating_sub(4)] - a * detrender[i.saturating_sub(6)]) * adj;
            i1[i] = detrender[i.saturating_sub(3)];

            ji[i] = (a * i1[i] + b * i1[i.saturating_sub(2)]
                - b * i1[i.saturating_sub(4)] - a * i1[i.saturating_sub(6)]) * adj;
            jq[i] = (a * q1[i] + b * q1[i.saturating_sub(2)]
                - b * q1[i.saturating_sub(4)] - a * q1[i.saturating_sub(6)]) * adj;
